...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (init_lib())
	{
		...
		try
		{
			...
			if ((isalib.dirnav() & CONNSTAT_ALL_PRESENT) != CONNSTAT_ALL_PRESENT) return EXIT_FAILED_TO_INIT_LIBRARY;
			res = EXIT_ABORTED;
			if (get_confirmation("IsalibVB.dll has been initialized. Press Y to execute payload...")) res = payload();
		}
